...
pub async fn inbound(
    ...
    if body.len() > max_bytes {
        return Err(ApiError::Events(horizons_core::events::Error::PayloadTooLarge));
    }
...
pub async fn list_subscriptions(
    ...
    let subs = state.event_bus.list_subscriptions(&org_id.to_string()).await?;
    Ok(Json(subs))
}
...
pub async fn unsubscribe(
    ...
    state.event_bus.unsubscribe(&org_id.to_string(), &id).await?;
